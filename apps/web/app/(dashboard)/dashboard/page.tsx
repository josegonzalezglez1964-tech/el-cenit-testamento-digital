      ...
      fetch('/api/testamento')
        ...
        .then((data) => {
          if (data.success && data.testamentos) {
            const existentes = new Set(testamentosLocal.map((t) => t.id));
            data.testamentos.forEach((t: TestamentoGuardado) => {
              if (!existentes.has(t.id)) {
                testamentosLocal.push(t);
              }
            });
          }
        })
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-500">
                        {t.herederos.length} herederos · {t.bienes.length} bienes ·{' '}
                        {t.bienes.reduce((s, b) => s + b.valorEstimado, 0).toLocaleString('es-ES')} €
                      </p>
